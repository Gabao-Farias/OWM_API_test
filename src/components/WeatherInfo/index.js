...
export default function WeatherInfo({info}){
    return(
        <WeatherContainer>
            <CityName>{info.weather.location.cityName}</CityName>
            ...
            <Ambient>
                <SmallCard>
                    ...
                    <SmallCardDescription>{Math.round(info.weather.ambient.temp_min) + "°C"}</SmallCardDescription>
                    <SmallerDescription>Mínima</SmallerDescription>
                </SmallCard>
                <SmallCard>
                    <MaterialIcon name="thermometer" size={30} color="#fff" />
                    <SmallCardDescription>{Math.round(info.weather.ambient.temp) + "°C"} </SmallCardDescription>
                    <SmallerDescription>Média</SmallerDescription>
                </SmallCard>
                <SmallCard>
                    <MaterialIcon name="thermometer-chevron-up" size={30} color="#fff" />
                    <SmallCardDescription>{Math.round(info.weather.ambient.temp_max) + "°C"} </SmallCardDescription>
                ...
                <SmallCard>
                    ...
                    <SmallCardDescription>{Math.round(info.weather.ambient.feels_like) + "°C"} </SmallCardDescription>
                    <SmallerDescription>Sensação térmica</SmallerDescription>
                </SmallCard>
                <SmallCard>
                    <MaterialIcon name="weather-windy" size={30} color="#fff" />
                    <SmallCardDescription>{Math.round(info.weather.ambient.wind_speed) + " Km/h"}</SmallCardDescription>
